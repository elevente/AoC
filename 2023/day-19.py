...
exampleInput = r"""px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
splitInput = exampleInput.split("\n\n")
workflows = {x.split("{")[0]: x[0:-1].split("{")[1].split(",") for x in splitInput[0].split("\n")}
parts = [{y.split("=")[0]: int(y.split("=")[1]) for y in x[1:-1].split(",")} for x in splitInput[1].split("\n")]


def goThroughWorkflow(part, workflow="in"):
    if workflow == "A":
        return True
    elif workflow == "R":
        return False
    ...
    for check in currentWorkflow:
        ...
        if len(splitCheck) == 1:
            return goThroughWorkflow(part, splitCheck[0])
        else:
            attribute = splitCheck[0][0]
            sign = splitCheck[0][1]
            measure = int(splitCheck[0][2:])
            target = splitCheck[1]
            if sign == ">":
                if part[attribute] > measure:
                    return goThroughWorkflow(part, target)
            else:
                if part[attribute] < measure:
                    return goThroughWorkflow(part, target)
...
def A():
    total = 0
    for part in parts:
        if goThroughWorkflow(part):
            total += sum(part.values())
    print(total)

# A()

def testWorkflow(workflow="in"):
    currentFlow = []
    if workflow == "A":
        currentFlow = True
    elif workflow == "R":
        ...
        for check in currentWorkflow:
            splitCheck = check.split(":")
            if len(splitCheck) == 1:
                currentFlow.append({ "next": testWorkflow(splitCheck[0]) })
            else:
                attribute = splitCheck[0][0]
                sign = splitCheck[0][1]
                measure = int(splitCheck[0][2:])
                target = splitCheck[1]
                currentFlow.append({ "attribute": attribute, "sign": sign, "measure": measure, "next": testWorkflow(target) })
    return currentFlow

def countSuccessfulFlows(possibleFlows, currentFlow):
    anti = []
    print(currentFlow)
    for flow in possibleFlows:
        if flow["next"] == True:
            if not("attribute" in flow):
                print("true 1", currentFlow)
                return currentFlow
            else:
                print("true 2", currentFlow, { "attribute": flow["attribute"], "sign": flow["sign"], "measure": flow["measure"] })
                return currentFlow.append({ "attribute": flow["attribute"], "sign": flow["sign"], "measure": flow["measure"] })
        elif flow["next"] == False:
            print("false")
            return False
        elif not("attribute" in flow):
            print("no", currentFlow)
            return countSuccessfulFlows(flow["next"], list(currentFlow))
        else:
            print("yes", currentFlow)
            return countSuccessfulFlows(flow["next"], list(currentFlow))
            # .append({ "attribute": flow["attribute"], "sign": flow["sign"], "measure": flow["measure"] })

def B():
    print(testWorkflow(), "\n")
    print(countSuccessfulFlows(testWorkflow(), []))
