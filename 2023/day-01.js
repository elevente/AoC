...
const input3 = `7zfhfmqmbkzrknxcfgxmqh`;

const input4 = `five8ninethreefive`;

const inputArr = input.split("\n");
const inputMatrix = inputArr.map(i => i.split(""));

/* let result = 0;

for (let i = 0; i < inputMatrix.length; i++) {
  const line = inputMatrix[i];
  const first = line.find(n => !isNaN(n));
  const last = line.findLast(n => !isNaN(n));
...
for (let i = 0; i < inputMatrix.length; i++) {
	const line = inputMatrix[i];
  const first = line.findIndex(n => !isNaN(n));
  const last = line.findLastIndex(n => !isNaN(n));
  if (first == -1) {
  	first = Infinity;
  }
  ...
  for (let j = 0; j < numbers.length; j++) {
  	const pos = line.join("").indexOf(numbers[j]);
    if (pos !== -1 && pos < firstWord) {
    	firstWord = pos;
      firstWordNum = j + 1;
    }
  	const pos2 = line.join("").lastIndexOf(numbers[j]);
    if (pos2 !== -1 && pos2 > lastWord) {
    	lastWord = pos2;
      lastWordNum = j + 1;
    }
  }
  console.log(first, last, firstWord, lastWord);
  const firstNum = first < firstWord
  	? line[first]
		: firstWordNum;
  const lastNum = last > lastWord
  	? line[last]
		: lastWordNum;
	result += parseInt(`${firstNum}${lastNum}`);
  console.log(`${line.join("")}: ${firstNum}${lastNum}`);
}
