...
for lineNum, lineText in enumerate(inputArr):
    for match in re.finditer(regex, lineText):
        ...
        for pos in range(length):
            if coordY > 0:
                neighborhood += inputArr[coordY - 1][coordX + pos]
            if coordY < len(inputArr) - 1:
                neighborhood += inputArr[coordY + 1][coordX + pos]
        if len(re.findall('\.', neighborhood)) < len(neighborhood):
            validNumbers.append(int(number))
        # print (number, neighborhood, len(re.findall('\.', neighborhood)) < len(neighborhood))
