...
gears = []
numbers = numpy.zeros([len(inputArr), len(inputArr[0])], dtype = int) 

for lineNum, lineText in enumerate(inputArr):
    for match in re.finditer(numregex, lineText):
        for r in range(len(match.group())):
            numbers[lineNum][match.start() + r] = match.group()

# print(numbers)

for lineNum, lineText in enumerate(inputArr):
    for match in re.finditer(regex, lineText):
        coordX = match.start()
        coordY = lineNum
        length = 1
        ...
        if coordY > 0:
            if coordX > 0:
                neighborhood1 += inputArr[coordY - 1][coordX - 1]
                if inputArr[coordY - 1][coordX - 1].isdigit():
                    neighborNumbers.append(numbers[coordY - 1][coordX - 1])
                    found1 = True
                neighborhood += inputArr[coordY - 1][coordX - 1]
            neighborhood1 += inputArr[coordY - 1][coordX]
            if not(found1) and inputArr[coordY - 1][coordX].isdigit():
                neighborNumbers.append(numbers[coordY - 1][coordX])
                found1 = True
            neighborhood += inputArr[coordY - 1][coordX]
            if coordX < len(lineText) - length:
                neighborhood1 += inputArr[coordY - 1][coordX + length]
            if not(found1) and inputArr[coordY - 1][coordX + 1].isdigit():
                neighborNumbers.append(numbers[coordY - 1][coordX + 1])
                found1 = True
                neighborhood += inputArr[coordY - 1][coordX + length]
        if (neighborhood1[0].isdigit() or neighborhood1[1].isdigit() or neighborhood1[2].isdigit()) and not(neighborhood1[0].isdigit() and not(neighborhood1[1].isdigit()) and neighborhood1[2].isdigit()):
            neighbors += 1
        elif neighborhood1[0].isdigit() and not(neighborhood1[1].isdigit()) and neighborhood1[2].isdigit():
            neighbors += 2
            neighborNumbers.append(numbers[coordY - 1][coordX + 1])
        neighborhood += " "
        if coordX > 0:
            neighborhood += inputArr[coordY][coordX - 1]
            if inputArr[coordY][coordX - 1].isdigit():
                neighbors += 1
                neighborNumbers.append(numbers[coordY][coordX - 1])
        neighborhood += " "
        if coordX < len(lineText) - length:
            neighborhood += inputArr[coordY][coordX + length]
            if inputArr[coordY][coordX + length].isdigit():
                neighbors += 1
                neighborNumbers.append(numbers[coordY][coordX + length])
        neighborhood += " "
        if coordY < len(inputArr) - 1:
            if coordX > 0:
                neighborhood2 += inputArr[coordY + 1][coordX - 1]
                if inputArr[coordY + 1][coordX - 1].isdigit():
                    neighborNumbers.append(numbers[coordY + 1][coordX - 1])
                    found2 = True
                neighborhood += inputArr[coordY + 1][coordX - 1]
            neighborhood2 += inputArr[coordY + 1][coordX]
            if not(found2) and inputArr[coordY + 1][coordX].isdigit():
                neighborNumbers.append(numbers[coordY + 1][coordX])
                found2 = True 
            neighborhood += inputArr[coordY + 1][coordX]
            if coordX < len(lineText) - length:
                neighborhood2 += inputArr[coordY + 1][coordX + length]
                if not(found2) and inputArr[coordY + 1][coordX + 1].isdigit():
                    neighborNumbers.append(numbers[coordY + 1][coordX + 1])
                    found2 = True
                neighborhood += inputArr[coordY + 1][coordX + length]
        if (neighborhood2[0].isdigit() or neighborhood2[1].isdigit() or neighborhood2[2].isdigit()) and not(neighborhood2[0].isdigit() and not(neighborhood2[1].isdigit()) and neighborhood2[2].isdigit()):
            neighbors += 1
        elif neighborhood2[0].isdigit() and not(neighborhood2[1].isdigit()) and neighborhood2[2].isdigit():
            neighbors += 2
            neighborNumbers.append(numbers[coordY + 1][coordX + 1])
        # print(neighborhood, neighbors)
        if neighbors == 2:
            # print(neighborNumbers)
            gears.append(neighborNumbers[0] * neighborNumbers[1])
