...
const pairs = input.split("\n\n").map(i => ({ left: eval(i.split("\n")[0]), right: eval(i.split("\n")[1]) }));
let indices = 0;
...
for (let i = 0; i < pairs.length; i++) {
	...
	for (let j = 0; j < item.left.length; j++) {
		const leftElem = item.left[j];
		const rightElem = item.right[j];
		let rightOrder = true;
		...
		if (Number.isInteger(leftElem) && Number.isInteger(rightElem)) {
			if (leftElem > rightElem) {
				rightOrder = false;
			}
		} else if (Array.isArray(leftElem) && Array.isArray(rightElem))		 {
